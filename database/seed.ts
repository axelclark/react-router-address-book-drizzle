...
async function seed() {
  ...
  // Create a one-off connection for seeding
  ...
  try {
    ...
    // Insert seed data
    const insertedContacts = await db.insert(contacts).values(seedContacts).returning();
    console.log(`✓ Inserted ${insertedContacts.length} contacts`);

    // Add some favorites
    const favoriteIds = insertedContacts.slice(0, 5).map(c => c.id);
    for (const id of favoriteIds) {
      await db.update(contacts).set({ favorite: true }).where(eq(contacts.id, id));
    }
    console.log(`✓ Marked ${favoriteIds.length} contacts as favorites`);
...
// Import eq for the favorites update
